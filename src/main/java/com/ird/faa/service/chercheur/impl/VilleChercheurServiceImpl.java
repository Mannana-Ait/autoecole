...
public List<Ville> findAll(){
    ...
    return villeDao.deleteByReference(reference);
    }
    @Override
    public Ville findByIdOrReference(Ville ville){
    Ville resultat=null;
    if(ville != null){
    if(StringUtil.isNotEmpty(ville.getId())){
    resultat= villeDao.getOne(ville.getId());
    }else if(StringUtil.isNotEmpty(ville.getReference())) {
    resultat= villeDao.findByReference(ville.getReference());
    }
    }
    return resultat;
    }
...
public Ville save (Ville ville){
    ...
    Ville result =null;
    Ville foundedVille = findByReference(ville.getReference());
...
public int delete(Ville ville){
    if(ville.getReference()==null) return -1;
